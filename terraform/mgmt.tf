...
EOT
  ...
  provisioner "file" {
    content     = templatefile("files/${var.cloud_provider}/acre.yaml.tmpl", { clouds = local.clouds, secure = local.secure, public = var.public, dns_domain = var.dns_domain, availability_zone = var.availability_zone, flavor_worker = var.flavor_worker, pw = random_password.gardener_password.result })
